...
3. **Diminishing returns:** Direct peer routing removes the coordinator relay,
   ...
   Three bounded variants preserved the known answer; further prompt-only
   tuning cannot provide a hard interrupt guarantee. A host adapter exposing
   steering/cancellation is required for that capability, and must be
