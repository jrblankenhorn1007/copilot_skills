...
1. **Before branch creation:** fetch `origin`, confirm `origin/main` is
   available, and create the worker's fresh branch from that ref. Follow the
   parent Ralph Loop skill's Git identity and authentication preflight first:
   fetch success proves read access only, so branch-push and merge permissions
   must be established through the repository's normal integration process.
   Preserve the worker branch and report a sanitized blocker if either write
   operation is denied; never pass credentials in worker instructions or
   change credential configuration without approval. Record the exact base
   SHA for the coordinator's ledger.
