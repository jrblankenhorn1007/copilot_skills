...
authoritative instructions; stale, unsupported, or
conflicting memory is not a requirement. Inspect the available tools and their
actual limits, such as repository access, test runners, agent dispatch, or
...
- Preserve the user's explicit scope: the objective, included work,
  exclusions, and requested deliverables. Paraphrase faithfully; do not silently narrow,
  broaden, replace, or reinterpret them.
...
- Do not invent work such as features, cleanup, refactors, deliverables, or
  acceptance criteria. Add only project constraints needed to carry out the explicit
  request safely and consistently with current instructions.
...
State the repository's required branch, commit, PR or no-PR process, status
handoff, remote-main verification, and post-merge memory-review condition. If
the worker must stop after a verified feature-branch publish and await
coordinator integration, say so; do not equate a pushed branch with
completion.
