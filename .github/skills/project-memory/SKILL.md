...
memory store rather than writing into this skill's repository. If that
repository has no memory convention, create its `.github/memory/` index and
only the category files needed for verified lessons, following this skill's
format and the repository's contribution rules.
