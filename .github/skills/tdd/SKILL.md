---
name: tdd
description: Use for behavior-changing implementation and bug fixes. Apply test-first Red-Green-Refactor and record evidence according to the active project's workflow.
---

# Test-Driven Development

Use this skill before writing production code for a behavior change or bug
fix. Follow any additional test requirements in the active project's
...
2. **Red: write the test first.** Add or change the test before production
   ...
   Record the command and meaningful failure in the project's designated
   progress log when its workflow requires one.
...
## Project-specific test guidance (`dj_maxxed_beats`)

The examples below come from `dj_maxxed_beats`. Apply them only when working
on that project; otherwise follow the active project's acceptance criteria
...
For each behavior-changing slice, include the following in the project's
designated progress log when its workflow requires one:
...
Commit tests with their implementation. Add any required decision-log entry
according to the active project's protocol. Never claim TDD evidence for a
